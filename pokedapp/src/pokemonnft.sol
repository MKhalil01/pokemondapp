...
contract PokemonNFT is ERC721A, VRFConsumerBaseV2, Ownable {
    ...
    // Events
    event MintRequested(address indexed minter, uint256 quantity, uint256 requestId);
    ...
    constructor(
        ...
        bytes32 _keyHash
    ) ERC721A("Pokemon NFT", "PKMN") VRFConsumerBaseV2(_vrfCoordinator) Ownable(msg.sender) {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for(uint256 i = 0; i < request.quantity; i++) {
            uint256 randomNum = uint256(keccak256(abi.encode(randomWords[0], i)));
    ...
    function selectPokemon(uint256 randomNum) internal returns (uint256) {
        ...
        if(rarity < 50) selectedRarity = 1; // Common
        else if(rarity < 80) selectedRarity = 2; // Uncommon
        else if(rarity < 95) selectedRarity = 3; // Rare
        else selectedRarity = 4; // Legendary

        while(attempts < 100) {
            uint256 pokemonId = uint256(keccak256(abi.encode(randomNum, attempts))) % TOTAL_POKEMON + 1;
            
            if(pokemonRarity[pokemonId] == selectedRarity) {
                if(selectedRarity == 4) { // Legendary
